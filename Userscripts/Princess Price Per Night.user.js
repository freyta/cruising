...
(function() {
    ...
    function updatePrices() {
        let rows = document.querySelectorAll('div[class="product-wrapper rounded-corners d-flex-column"]');

        rows.forEach((row) => {
            const priceDiv = row.querySelector('div[class="price align-center"]');
            const priceText = parseFloat(priceDiv.textContent.trim().replace("$","").replace(",","")); // Convert price to number
            const textReplace = row.querySelector('div[class="gotham-book"]'); // Target the correct container for price-per-night

            const daysDivs = row.querySelector('a[class="plain-text-btn invert-decoration gotham-xn-book text-left"]').textContent.trim();
